...
public partial class TelemetryLogger : MonoBehaviour
{
    ...
    public void Log<T>( string eventType, T data) {
        ...
        var t = typeof(T);
        if(!t.IsSerializable) {
            Debug.LogError($"Trying to log event '${eventType}' with non-serializable data payload type ${t.Name}. Be sure to use the [System.Serializable] attribute on any custom class/struct you want to use as telemetry data.");
        }
    #endif
